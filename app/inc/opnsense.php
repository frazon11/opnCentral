...
function opn_request(array $f,string $path,string $method='GET',?array $payload=null,int $timeout=20): array {
 ...
 curl_setopt_array($ch,$o);$b=curl_exec($ch);$e=curl_error($ch);$s=(int)curl_getinfo($ch,CURLINFO_RESPONSE_CODE);
...
function opn_download(array $f,string $path): string {
 ...
 $b=curl_exec($ch);$e=curl_error($ch);$s=(int)curl_getinfo($ch,CURLINFO_RESPONSE_CODE);
