...
if($_SERVER['REQUEST_METHOD']==='POST'){require_csrf();if(hash_equals(envv('ADMIN_USER','admin'),(string)($_POST['username']??''))&&hash_equals(envv('ADMIN_PASSWORD'),(string)($_POST['password']??''))){session_regenerate_id(true);$_SESSION['auth']=true;$_SESSION['username']=(string)($_POST['username']??'');header('Location: /');exit;}usleep(350000);$error=t('login.invalid');}
