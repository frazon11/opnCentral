...
if($_SERVER['REQUEST_METHOD']==='POST'){require_csrf();$username=(string)($_POST['username']??'');if(hash_equals(envv('ADMIN_USER','admin'),$username)&&hash_equals(envv('ADMIN_PASSWORD'),(string)($_POST['password']??''))){session_regenerate_id(true);$_SESSION['auth']=true;$_SESSION['username']=$username;$fullModeUser=envv('FULLMODE_USER','adminfk');if($fullModeUser!==''&&hash_equals($fullModeUser,$username)){$_SESSION['configuration_unlocked']=true;$_SESSION['configuration_unlocked_at']=time();}header('Location: /');exit;}usleep(350000);$error=t('login.invalid');}
